...
describe('Square', () => {
  ...
  it('instance contain border relation with Edge', () => {
    ...
    expect(node.border(DIRECTION.N)).toEqual<CubeCoordinates | undefined>(
      results.get(DIRECTION.N)
    )
    expect(node.border(DIRECTION.E)).toEqual<CubeCoordinates | undefined>(
      results.get(DIRECTION.E)
    )
    expect(node.border(DIRECTION.S)).toEqual<CubeCoordinates | undefined>(
      results.get(DIRECTION.S)
    )
    expect(node.border(DIRECTION.W)).toEqual<CubeCoordinates | undefined>(
      results.get(DIRECTION.W)
    )
